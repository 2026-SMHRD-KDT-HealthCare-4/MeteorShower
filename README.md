...
| 오늘의 운동 | 처방받은 운동 목록·진행 상태 확인, 각 카드에서 운동 타입·손 방향별 가이드 미리보기 영상 재생 (`GET /patients/me/today-exercises`, `frontend/public/videos/*.mp4`) |
...
| 실시간 운동 세션 | 브라우저가 `getUserMedia`로 카메라를 직접 열고 100ms마다 JPEG 프레임을 AI 서버로 전송해 분석 결과를 받음 (`ExerciseSession.jsx`, AI 8000 WebSocket, `use_client_frames: true`). AI 서버에 웹캠 하드웨어가 없어도 동작. 세션 결과 HTTP 저장이 완료될 때까지 "돌아가기"/"다음 운동" 버튼이 비활성화되며, 연속 운동 시 이전 WS를 3.5초 더 열어 GIF 캡처를 받은 뒤 닫음 |
...
│   │                                # 브라우저 프레임 수신용 QueueVideoCapture와 로컬 웹캠 fallback 모두 지원
...
│   │                                # action:"frame" 수신 시 브라우저 JPEG를 QueueVideoCapture로 전달
...
│   ├── export_guide_videos.py       # 가이드 미리보기 MP4 영상 생성 도구 (frontend/public/videos/에 출력)
...
│   ├── Dockerfile / docker-compose.yml
│   └── src/
│       ├── public/videos/           # 가이드 미리보기 영상 (grip_left/right.mp4, tapping_left/right.mp4)
...
- PostgreSQL 인스턴스
- 웹캠 (AI 서버가 직접 접근하는 것이 아니라, 브라우저 `getUserMedia` API가 접근합니다)

> **카메라 권한과 HTTPS**: 브라우저의 `getUserMedia`는 `localhost` 또는 HTTPS 오리진에서만 동작합니다. 로컬 개발 시 `http://localhost:5173`으로 접근하면 정상 동작합니다. 배포 환경(`https://fine123.site`)은 HTTPS라 문제 없습니다. 프록시·IP 직접 접근 등 non-localhost HTTP 환경에서는 카메라 권한을 얻을 수 없습니다.
...
- `ai/docker-compose.yml`에는 웹캠 디바이스 마운트가 없습니다. 브라우저 카메라 프레임 기반 분석 모드(`use_client_frames: true`)로 항상 동작하므로, AI 서버 컨테이너가 호스트 웹캠에 직접 접근할 필요가 없습니다.
- `backend/Dockerfile`은 `--reload` 없이 `uvicorn main:app --host 0.0.0.0 --port 8000`으로 실행합니다. `backend/docker-compose.yml`이 컨테이너 내부 8000번을 호스트 8001번으로 매핑하며, 아래 추가 마운트가 있습니다.
  - `./firebase-service-account.json:/app/firebase-service-account.json:ro` — Firebase Admin SDK 서비스 계정 키
  - `../ai:/project/ai:ro` — LLM 리포트 생성 시 `ai/llm_client.py` 접근용
  - 환경변수 `PROJECT_ROOT=/project` — `llm_report_generator.py`가 AI 모듈 경로를 찾기 위해 사용
- `frontend/Dockerfile`은 멀티스테이지 빌드입니다. `node:20-alpine`에서 `npm run build`로 정적 파일을 생성한 뒤 `nginx:alpine`으로 옮겨 `nginx.conf`(SPA 라우팅을 위한 `try_files` 설정)와 함께 80번 포트로 서빙합니다. 빌드 시점에 `VITE_API_URL`/`VITE_WS_URL`을 `ARG`로 주입하며 `frontend/docker-compose.yml`에 배포 도메인이 확정되어 있습니다(`VITE_API_URL: https://fine123.site/api`, `VITE_WS_URL: wss://fine123.site/ws`). 포트는 `5173:80`(호스트 5173 → 컨테이너 nginx 80)으로 매핑됩니다.
...
| 서비스 | 내부 포트 | 호스트 포트 | 비고 |
|--------|-----------|-------------|------|
| ai | 8000 | 8000 | 웹캠 마운트 불필요 (브라우저가 카메라를 처리하는 `use_client_frames` 모드로 동작) |
| backend | 8000 | 8001 | `./app:/app`, `./firebase-service-account.json`, `../ai:/project/ai:ro` 마운트, `PROJECT_ROOT=/project` 환경변수 |
| frontend | 80 | 5173 | `npm run build` 결과를 nginx로 서빙. `VITE_API_URL=https://fine123.site/api`, `VITE_WS_URL=wss://fine123.site/ws` |

ai와 backend가 각각 8000번 내부 포트를 쓰지만, backend를 8001로 외부 매핑해 충돌을 피합니다. frontend는 nginx가 80번 포트로 정적 파일을 서빙하고 호스트의 5173으로 노출합니다.
...
`ai/hand_tracking.py`가 생성한 GIF 캡처는 `ExerciseSession.jsx`의 `uploadCaptureGifs()`가 받아 `POST /patients/me/exercise-sessions/{rehab_session_id}/captures`로 백엔드에 업로드하고, 백엔드는 Firebase Storage에 올린 뒤 URL을 `rehab_exercise_capture`에 저장합니다. 의사 화면(`DailyReport.jsx`)은 이 데이터를 `capture_gifs`로 조회합니다. GIF는 `session_end` 이후 약 2~2.5초 뒤 AI 서버에서 전송되므로, 프론트엔드는 이전 WebSocket을 3.5초 더 열어두고 GIF를 수신한 뒤 닫는 방식으로 연속 운동 시 GIF 유실을 방지합니다. `rehab_exercise_capture.set_first_gif_url`은 nullable로 변경되어(마이그레이션 `l7g8h9i0j1k2`) 연속 운동 전환 시 첫 GIF가 없는 경우도 허용합니다. `firebase-admin==7.4.0`이 `backend/venv`에 설치되어 있으며 파이프라인은 정상 동작합니다.

`ai/docker-compose.yml`에는 웹캠 디바이스 마운트가 없습니다. 브라우저 카메라 프레임 기반 분석 모드로 전환되어 AI 서버가 웹캠에 직접 접근할 필요가 없어진 결과입니다.

`frontend/docker-compose.yml`의 `VITE_API_URL`/`VITE_WS_URL`이 `https://fine123.site`로 하드코딩되어 있습니다. 이 파일을 그대로 써서 `docker-compose up`으로 프론트엔드를 빌드하면 API 요청이 모두 배포 서버로 날아가고 로컬 백엔드와 연결되지 않습니다. 로컬에서 Docker로 프론트엔드를 빌드하려면 해당 값을 `http://localhost:8001`/`ws://localhost:8000/ws`로 변경해야 합니다.
...
- 브라우저 카메라 프레임 큐(`QueueVideoCapture`)에서 프레임을 가져오지 못하면 루프가 안전하게 종료됩니다.
- 카메라가 다른 프로그램에서 점유 중일 때(`NotReadableError`) 프론트엔드가 구체적인 안내 메시지("AI 서버/카메라 앱/다른 브라우저 탭을 종료한 뒤 다시 시도하세요")를 표시합니다.
