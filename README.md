...
| 실시간 운동 세션 | 브라우저가 `getUserMedia`로 카메라를 직접 열고 100ms마다 JPEG 프레임을 AI 서버로 전송해 분석 결과를 받음 (`ExerciseSession.jsx`, AI 8000 WebSocket, `use_client_frames: true`). AI 서버에 웹캠 하드웨어가 없어도 동작하며, `frame_queue`가 없을 경우 `cv2.VideoCapture` fallback도 코드에 남아있음 |
