...
> **참고**: Pillow는 `ai/requirements.txt`에 누락되어 있습니다. 실행 환경에 Pillow가 설치되지 않으면 과부하 GIF 캡처 기능이 동작하지 않습니다.
...
`ai/`, `backend/`, `frontend/` 세 디렉토리 모두 자체 `Dockerfile`과 `docker-compose.yml`을 갖고 있습니다. 다만 세 구성 모두 로컬 venv 실행과 차이가 있어 그대로 쓰기는 어렵습니다.

- `ai/Dockerfile`은 `ai/requirements.txt`를 사용하지 않고 일부 패키지만 직접 설치합니다. `openai`, `python-jose`, `httpx`, `python-dotenv`, `Pillow`가 빠져 있어 해당 기능이 동작하지 않을 수 있습니다.
- `ai/docker-compose.yml`은 `devices: /dev/video0:/dev/video0`로 웹캠을 매핑하는데, 이는 Linux 전용 경로라 Windows 호스트에서는 동작하지 않습니다.
- `backend/docker-compose.yml`은 컨테이너 내부 8000번을 호스트 8001번으로 매핑합니다.
- `frontend/Dockerfile`은 `npm run dev -- --host`로 Vite 개발 서버를 컨테이너에서 그대로 실행합니다(프로덕션 빌드가 아닙니다).
...
| 서비스 | 내부 포트 | 호스트 포트 | 비고 |
|--------|-----------|-------------|------|
| ai | 8000 | 8000 | `/dev/video0` 마운트로 웹캠 접근 (Linux 전용) |
| backend | 8000 | 8001 | 앱 코드(`./app:/app`) 볼륨 마운트 |
| frontend | 5173 | 5173 | Vite 개발 서버 모드 |

ai와 backend가 각각 8000번 내부 포트를 쓰지만, backend를 8001로 외부 매핑해 충돌을 피합니다. ai 컨테이너가 `/dev/video0`를 마운트하는 이유는 Linux 환경에서 웹캠 디바이스에 접근하기 위해서입니다.
...
`backend/app/routers/ws.py`의 `/ws`는 받은 텍스트를 그대로 돌려보내는 echo 더미 구현입니다. 실제 동작 분석은 `ai/websocket_server.py`(포트 8000)를 통해서만 이루어집니다.

Pillow(`PIL`)는 `ai/requirements.txt`에 누락되어 있습니다. 누락된 상태로 AI 서버를 설치하면 과부하 발생 시 GIF 캡처를 생성하는 단계에서 `ImportError`가 발생합니다.
...
세 서비스의 Docker 구성은 로컬 venv 실행과 패키지 목록·포트 매핑이 달라 그대로는 동작하지 않을 수 있습니다 — 자세한 내용은 [Docker로 실행하기](#docker로-실행하기) 참고.
