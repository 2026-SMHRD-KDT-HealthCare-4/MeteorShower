...
const summaryStats = [
  { label: '이번 주 총 수행 횟수', value: '342회', icon: 'fitness_center', color: 'text-doctor-primary', bg: 'bg-[#e8f0fe]' },
  { label: '평균 일치율', value: '87%', icon: 'target', color: 'text-[#1a73e8]', bg: 'bg-[#e8f0fe]' },
  { label: '연속 수행일', value: '12일', icon: 'local_fire_department', color: 'text-[#006398]', bg: 'bg-[#cce5ff]' },
  { label: '전주 대비 향상', value: '+12%', icon: 'trending_up', color: 'text-doctor-primary', bg: 'bg-[#e8f0fe]' },
];
            ...
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-stack-md md:p-stack-lg space-y-stack-lg shadow-card">
              ...
              <div className="space-y-stack-sm">
                <div className="flex justify-between items-end">
                  <h3 className="text-title-md font-bold text-doctor-primary flex items-center gap-2">
                    <span className="material-symbols-outlined text-xl">target</span>
                    일치율 (Accuracy)
                  </h3>
                  <span className="text-label-sm text-doctor-primary bg-doctor-primary/10 px-2 py-0.5 rounded-full font-semibold">+12% vs last week</span>
                </div>
              ...
              <div className="space-y-stack-sm">
                ...
                <LineChart data={repData} color="#006398" />
              </div>
                ...
                <div className="w-full h-[180px] bg-white border border-outline-variant rounded-lg p-4 flex items-end gap-2">
                  {[85, 70, 90, 60, 95, 45, 80].map((pct, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div
                        className="w-full rounded-t-md bg-doctor-primary transition-all duration-700"
                ...
                {[
                  ...
                  <div key={ex.name}>
                    <div className="flex justify-between text-label-md mb-1">
                      ...
                      <span className="text-doctor-primary font-semibold">{ex.value}%</span>
                    </div>
                    <div className="w-full bg-surface-container h-2 rounded-full">
                      <div className="bg-doctor-primary h-full rounded-full transition-all duration-700" style={{ width: `${ex.value}%` }} />
                    </div>
                  </div>
                ))}
              </div>
