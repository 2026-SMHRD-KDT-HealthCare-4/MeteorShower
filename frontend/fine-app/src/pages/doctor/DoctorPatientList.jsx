...
export default function DoctorPatientList() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#fbf9f8' }}>
      ...
      <main className="max-w-[1280px] mx-auto px-4 md:px-10 py-8 space-y-6 flex flex-col" style={{ minHeight: 'calc(100vh - 64px)' }}>
        {/* Search + Register */}
        <section className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="relative w-full md:max-w-xl">
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-outline hover:text-error transition-colors">cancel</button>
            )}
          </div>
          <button className="text-white px-6 py-3 rounded-xl text-label-md font-semibold flex items-center gap-2 shadow-md hover:opacity-90 transition-opacity" style={{ background: '#1a73e8' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
          ...
          <section className="space-y-4 flex flex-col">
            <div className="flex items-center gap-3 px-2">
              ...
              <span className="ml-auto bg-surface-container-high text-on-surface-variant px-2.5 py-0.5 rounded-full text-label-sm font-semibold">{allPatients.length}</span>
            </div>
          ...
          <section className="space-y-4 flex flex-col">
            <div className="flex items-center gap-3 px-2">
              ...
              <span className="ml-auto bg-secondary-fixed text-on-secondary-fixed-variant px-2.5 py-0.5 rounded-full text-label-sm font-semibold">{waitingPatients.length}</span>
            </div>
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[500px] pr-1">
          ...
          <section className="space-y-4 flex flex-col">
            <div className="flex items-center gap-3 px-2">
              ...
              <span className="ml-auto bg-tertiary-fixed text-on-tertiary-fixed-variant px-2.5 py-0.5 rounded-full text-label-sm font-semibold">{prescribedPatients.length}</span>
            </div>
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[500px] pr-1">
      ...
      {/* Ambient glow */}
      <div className="fixed top-0 right-0 -z-10 w-1/3 h-1/3 blur-[120px] rounded-full pointer-events-none" style={{ background: 'rgba(26,115,232,0.05)' }} />
      <div className="fixed bottom-0 left-0 -z-10 w-1/4 h-1/4 blur-[100px] rounded-full pointer-events-none" style={{ background: 'rgba(72,88,171,0.05)' }} />
    </div>
  );
}
