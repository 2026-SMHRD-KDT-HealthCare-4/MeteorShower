...
# 가이드 홀로그램 밝기 적응 — 배경이 밝을수록(b↑) 불투명도/채도를 높이고 명도는 낮춰서
# 얼굴/창문 같은 밝은 영역 위에서도 진한 파랑으로 또렷하게 도드라지게 한다.
HOLOGRAM_ALPHA_BASE             = 0.75
HOLOGRAM_ALPHA_BRIGHTNESS_GAIN  = 0.85
HOLOGRAM_ALPHA_MIN              = 0.75
HOLOGRAM_ALPHA_MAX              = 1.60
HOLOGRAM_SAT_BASE               = 1.00
HOLOGRAM_SAT_BRIGHTNESS_GAIN    = 1.20
HOLOGRAM_SAT_MIN                = 1.00
HOLOGRAM_SAT_MAX                = 2.20
HOLOGRAM_VAL_BASE               = 1.15
HOLOGRAM_VAL_BRIGHTNESS_GAIN    = -0.40  # 배경 밝을수록(b↑) 명도를 낮춰 진한 톤으로 대비를 만듦
HOLOGRAM_VAL_MIN                = 0.75
HOLOGRAM_VAL_MAX                = 1.15
...
def draw_animated_guide(frame, guide_frame_idx, guide_np):
    ...
    # 배경이 밝을수록 명도(V)는 낮춰서 옅게 번지지 않고 진한 파랑으로 대비를 만든다.
